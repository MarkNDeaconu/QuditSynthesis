...
def update_visualization(central_node):
    global G, pos, central_node_label, neighbor_nodes_info, starting_node, edge_label_dict, fig, ax, current_hover, labels, node_labels
    ...
    for info in neighbor_nodes_info:
        neighbor = info['node']
        if neighbor.__gt__(central_node) and not neighbor.__lt__(central_node):
